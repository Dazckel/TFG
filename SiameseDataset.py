...
class SiameseNetworkDataset(Dataset):
    def __init__(self, files_and_clases=None, imageFolderDataset=PATH_DATASET, path_root=PATH_ROOT, transform=None,
        ...
        else:  # En caso de que hayamos creado ya el dataset
    ...
    def createSamples(self):
        ...
        filenames_classes = dict(zip(self.files, self.classes))
        file_pairs = list(combinations(self.files, 2))
...
def createDataset(path_dataset):
    ...
    minNInstances = np.max([number_NL, number_MCI, number_AD])
    ...
    files_Train = [filenames[random_idxs[i]] for i in range(0, minNInstances)]
    class_Train = [classes[random_idxs[i]] for i in range(0, minNInstances)]
